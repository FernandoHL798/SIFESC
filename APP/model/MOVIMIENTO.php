...
class MOVIMIENTO extends CONEXION{
    ...
    public function queryconsultaMovimiento($idInscripcion){
        $query="SELECT e.semestre as semestreAlumno, ins.id_periodo_fk, asi.id_asignacion, g.id_grupo, g.nombre_grupo, a.id_asignatura, a.codigo, a.nombre, a.creditos, a.semestre as semestreMat, mov.id_inscripcion_fk, mov.estatus, p.maximo_materias FROM `estudia` e, `inscripcion` ins, `movimiento` mov, `asignacion` asi, `asignaturas` a, `grupos` g, plandeestudios p where ins.id_inscripcion=mov.id_inscripcion_fk and mov.id_asignacion_fk=asi.id_asignacion and asi.id_grupo_fk=g.id_grupo and g.id_asignatura_fk=a.id_asignatura and a.id_plan_fk=p.id_plan and e.id_usuarioalumno_fk=ins.id_usuarioalumno_fk and ins.id_inscripcion=".$idInscripcion;
        $this->connect();
